angular.module('angularPayments', []);;angular.module('angularPayments')
    ...
    .factory('_Format', ['Cards', 'Common', '$filter', function (Cards, Common, $filter) {
        ...
        var _formats         = {},
            _hasTextSelected = function ($target) {
                var ref;
    ...
    .factory('_Validate', ['Cards', 'Common', '$parse', function (Cards, Common, $parse) {
        ...
        _validators['cvc'] = function (cvc, ctrl, scope, attr) {
            ...
            if (type) {
                ref = cvc.length;
                return __indexOf.call((ref1 = Cards.fromType(type)) != null ? ref1.cvcLength : void 0, ref) >= 0;
            } else {
        ...
        _validators['card'] = function (num, ctrl, scope, attr) {
            ...
            ref = num.length;
            ret = (__indexOf.call(card.length, ref) >= 0) && (card.luhn === false || _luhnCheck(num));
        ...
        return function (type, val, ctrl, scope, attr) {
            if (!_validators[type]) {
                throw 'Unknown type for validation: "' + type + '". ' +
                'Should be one of: "' + Object.keys(_validators).join('", "') + '"';
            }
    ...
    .directive('paymentsValidate', ['$window', '_Validate', '_ValidateWatch',
